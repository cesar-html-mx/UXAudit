...
## D-043 — Isolated post-v0.1 continuation with immutable rollback anchors

- Date: 2026-07-31
- Status: accepted
- Context: Realistic consumer exploration exposed that the documented v0.1.0 model records custom
  JSX syntax but does not resolve imported component definitions or rendered composition. The
  maintainer approved a six-hour feasibility attempt only if the published release, historical
  harness evidence, and existing external demo remain recoverable without history rewriting.
- Decision: Base M07 on public `main` commit `de540f0`, work only on
  `milestone/m07-component-graph`, and retain local immutable tags
  `safeguard/pre-v0.2-main-20260731` and
  `safeguard/post-harness-evidence-20260731`. Restore the internal harness only on the milestone
  branch and preserve M01-M06 unchanged. Do not modify the external `uxaudit-demo-mercado-raiz`
  project. Treat 2026-07-31T15:38:04-06:00 as the M07 go/no-go deadline.
- Alternatives considered: changing `main` directly; rewriting M03/M04/M06 history; extending the
  existing demo first; or promising broad React interpretation before a technical spike.
- Consequences: The experiment can be discarded without changing v0.1.0, `main`, npm, historical
  branches, or the demo. Internal harness files may appear on the milestone branch but must not be
  included in a clean public release diff.
