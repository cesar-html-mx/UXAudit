...
| RF-09       | Rule metadata and RuleLoader                            | M04         | `tests/domain/rules/rule-contracts.test.ts` plus category/rule filter tests                                                     |
| RF-10       | `Rule` and RuleEvaluator                                | M04         | model-only rule contract test plus model-driven integration tests                                                               |
| RF-11       | `RuleFinding`, `Finding`, and rule implementations      | M04         | normalization contract plus zero/one/multiple finding cases                                                                     |
...
| RNF-05      | `Finding`/`Rule`                                        | M04         | defensive full `SourceLocation`, rule identity, and classification assertions                                                   |
...
| RNF-10      | Rule engine/reporters                                   | M04/M05     | report-independent result contract and later reporter consistency without reevaluation                                          |
