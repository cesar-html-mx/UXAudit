...
| RF-08       | `src/domain/models/`, `extract-babel-analysis.ts`       | M03         | AST-free contract tests plus `extract-babel-analysis.test.ts`: components, JSX kinds, relationships, values, text, serialization |
...
| RF-12       | `SourceLocation`, extraction and finding location       | M03/M04     | half-open relative-path/UTF-16 contracts plus exact file/component/JSX/attribute/property extraction location assertions         |
...
| RNF-04      | Pipeline                                                | M02-M06     | unit/pipeline reruns, byte-identical scenario JSON, and deterministic AST-free extraction serialization                          |
