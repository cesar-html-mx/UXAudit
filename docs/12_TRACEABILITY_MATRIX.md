...
| RF-15       | `Reporter` and terminal/JSON/HTML adapters              | M05       | exact terminal color/no-color/threshold/hostile tests; JSON/HTML consistency scenario follows in T04-T05                        |
...
| RNF-04      | scan/analyze/configuration, engine, and result          | M02-M06   | prior deterministic pipelines plus canonical frozen configuration/result rebuilds and reporter reruns                           |
...
| RNF-09      | Node.js 24 package engines, portable paths, CI matrix   | M01-M06   | cross-platform workflow plus POSIX/Windows configuration-open flags and portable output-directory matrix                        |
| RNF-10      | Rule engine/`AuditResult`/reporters                     | M04/M05   | frozen normalized data plus pure deterministic terminal rendering without parser/rule reevaluation or input mutation            |
