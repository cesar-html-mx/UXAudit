...
| RF-09       | Rule registry and `loadRules`                           | M04         | `tests/rules/rule-registry.test.ts` and `tests/rules/load-rules.test.ts`                                                        |
...
| RF-11       | `RuleFinding`, `Finding`, and rule implementations      | M04         | evaluator cases plus A11Y zero/one/multiple positive, negative, and unsupported fixtures                                        |
| RF-12       | `SourceLocation`, extraction and model validation       | M03/M04     | exact extraction locations plus A11Y half-open range and canonical-provenance assertions                                        |
| RF-13       | `RuleMetadata` and normalized `Finding`                 | M04         | `tests/domain/rules/rule-contracts.test.ts` explanation/recommendation/reference assertions                                     |
| RF-14       | Rule/finding classification and AuditResult             | M04/M05     | stable high-severity A11Y metadata/integration plus later grouping and ordering                                                 |
