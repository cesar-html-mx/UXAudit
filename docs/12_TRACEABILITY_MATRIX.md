...
| RF-15       | `Reporter` and terminal/JSON/HTML adapters              | M05       | exact terminal; lossless JSON/schema; shared writer; standalone escaped HTML; cross-reporter identity and scenario tests        |
...
| RNF-10      | Rule engine/`AuditResult`/reporters                     | M04/M05   | frozen normalized data, pure deterministic terminal/JSON/HTML, cross-format identity, and separate persistence without reruns   |
