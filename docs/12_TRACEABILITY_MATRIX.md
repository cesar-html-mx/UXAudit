...
| RF-09       | Initial rule registry and `loadRules`                   | M04         | engine filter tests plus the exact eight-rule three/two/two/one category registry assertion                                     |
| RF-10       | `Rule` and `evaluateRules`                              | M04         | model-only contract plus `tests/rules/evaluate-rules.test.ts` exact-once/isolation assertions                                   |
| RF-11       | `RuleFinding`, `Finding`, and rule implementations      | M04         | all eight rules cover zero/one/multiple, positive, negative, boundary, and unsupported fixtures                                 |
| RF-12       | `SourceLocation`, extraction and model validation       | M03/M04     | exact extraction plus canonical finding provenance at element and retained style-property ranges                                |
| RF-13       | `RuleMetadata` and normalized `Finding`                 | M04         | complete explanation/recommendation/reference/limitation assertions for contracts and every stable rule                         |
| RF-14       | Rule/finding classification and AuditResult             | M04/M05     | four-category stable catalog metadata/integration plus later grouping and ordering                                              |
...
| RNF-02      | Rule/Finding/Reporter interfaces                        | M04/M05     | typed model-only test rule and later reporter extension without core modification                                               |
...
| RNF-04      | scan/analyze pipeline, model builder, and rule engine   | M02-M06     | deterministic registry/filter/finding order and repeated evaluation serialization plus prior pipeline reruns                    |
...
| RNF-10      | Rule engine/reporters                                   | M04/M05     | normalized findings/errors/counters without presentation imports and later reporter consistency                                 |
