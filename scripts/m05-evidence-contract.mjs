...
const exactJsonRelativePaths = new Set([
  'scenario/audit-report.json',
  'scenario/audit-result-expected.json',
]);
...
export const assertEvidenceJsonIsCanonical = async (directory) => {
  for (const filePath of await findEvidenceFiles(directory)) {
    ...
    const expected =
      exactJsonRelativePaths.has(relativePath) ||
      exactJsonRelativePaths.has(`scenario/${relativePath}`)
        ? `${JSON.stringify(parsed, null, 2)}\n`
        : await format(content, jsonFormatOptions);
