...
const fixtureDirectory = path.join(repositoryRoot, 'tests', 'fixtures', 'm04-rules');
...
const throwingRule = {
  evaluate: () => {
    throw new Error('M04_SCENARIO_PRIVATE_RULE_FAILURE');
  },
  metadata: {
    category: 'ux',
    ...
    limitations: ['This rule exists only inside the controlled M04 scenario.'],
...
const temporaryRoot = await mkdtemp(path.join(tmpdir(), 'uxaudit-m04-scenario-'));
