...
const evidenceDirectory = path.join(evidenceParentDirectory, 'm06-validation');
...
const assertEvidenceDestinationIsAuthorized = async () => {
  ...
  if (
    ...
    throw new Error('M06 evidence destination is not a regular in-repository directory.');
  }
  ...
  if (
    ...
    evidenceRealPath !== path.join(evidenceParentRealPath, 'm06-validation')
  ) {
    throw new Error('M06 evidence destination resolves outside the repository.');
  }
  ...
  if (
    ...
    throw new Error('M06 evidence destination changed during authorization.');
  }
  ...
  if (
    ...
    throw new Error('M06 evidence manifest or report resolves outside the approved destination.');
  }
  ...
  if (
    ...
    throw new Error('M06 evidence manifest and report must be regular files.');
  }
...
const assertAuthorizationIsUnchanged = (expected, observed) => {
  if (
    ...
    throw new Error('M06 evidence destination changed before manifest publication.');
  }
};
...
const assertPreexistingBaseManifestIsValid = async (finalManifest) => {
  ...
  if (
    ...
    throw new Error(
      `Pre-existing evidence manifest does not match the ${String(
        baseEvidenceArtifactCount,
      )} base artifacts.`,
    );
  }
...
const assertMilestoneReportIsComplete = async () => {
  const content = await readFile(milestoneReport, 'utf8');
  ...
  if (
    ...
    requiredFragments.some((fragment) => !content.includes(fragment))
  ) {
    throw new Error('M06 milestone report is empty, malformed, or missing required closure facts.');
  }
};

await access(milestoneReport);
await assertMilestoneReportIsComplete();
...
console.log('M06 evidence finalization: PASS');
