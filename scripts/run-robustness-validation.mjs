...
const scenarioId = 'UXAUDIT-SYSTEM-ROBUSTNESS-SECURITY-PERFORMANCE';
...
const main = async () => {
  ...
  assert.equal(manifest.corpusId, 'UXAUDIT-CONTROLLED-PROJECTS');
  assert.equal(await pathExists(cliPath), true, 'Build the CLI before running the scenario.');
  const temporaryRoot = await mkdtemp(path.join(tmpdir(), 'uxaudit-robustness-validation-'));
  const cases = [];
  ...
  try {
    ...
    await writeFile(
      ...
      await toCanonicalJson({
        name: 'uxaudit-system-deep-fixture',
        private: true,
        scripts: {
          'uxaudit:sentinel':
