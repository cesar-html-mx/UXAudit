...
const scenarioId = 'UXAUDIT-RULE-ACCURACY';
...
const temporaryRoot = await mkdtemp(path.join(tmpdir(), 'uxaudit-accuracy-validation-'));

try {
  const [{ content: groundTruthContent, value: groundTruth }, { value: manifest }] =
    await Promise.all([
      readCanonicalJson(groundTruthPath, 'UXAudit accuracy ground truth'),
      readCanonicalJson(manifestPath, 'UXAudit controlled-project manifest'),
    ]);

  assert.equal(groundTruth.schemaVersion, schemaVersion);
  assert.equal(groundTruth.groundTruthId, 'UXAUDIT-RULE-ACCURACY-GROUND-TRUTH');
