...
const scenarioId = 'UXAUDIT-CONTROLLED-PROJECTS';
const repositoryRoot = process.cwd();
const fixtureRoot = path.join(repositoryRoot, 'fixtures', 'system-validation');
...
const readManifest = async () => {
  ...
  assert.equal(
    content,
    await toCanonicalJson(manifest),
    'The system-validation manifest must use canonical JSON.',
  );
  assertExactKeys(
    manifest,
    [
...
const createLargeProject = async (projectRoot, definition) => {
  ...
  await writeFile(
    ...
    await toCanonicalJson({
      name: 'uxaudit-system-large-fixture',
      private: true,
      scripts: {
...
const temporaryRoot = await mkdtemp(path.join(tmpdir(), 'uxaudit-system-scenario-'));
...
try {
  ...
  assert.equal(
    firstJson,
    secondJson,
    'Repeated system-validation projections must be byte-identical.',
  );
