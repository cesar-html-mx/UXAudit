...
const createControlledAuditResultRequest = (color = false) => {
  ...
  const findings = [
    ...
    finding({
      ...
      reference: {
        ...
        url: 'https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html?source=m05&mode=evidence',
      },
  ...
  return {
    ...
    toolVersion: `0.1.0-m05-${hostileText}`,
  };
};
...
const validateXssStructure = (html) => {
  ...
  assert.deepEqual(hrefs, [
    'https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html?source=m05&amp;mode=evidence',
  ]);
...
const temporaryRoot = await mkdtemp(path.join(tmpdir(), 'uxaudit-m05-scenario-'));
