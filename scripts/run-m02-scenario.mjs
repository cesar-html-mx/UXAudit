...
const temporaryRoot = await mkdtemp(path.join(tmpdir(), 'uxaudit-m02-scenario-'));
...
const createControlledProject = async () => {
  ...
  await Promise.all([
    writeFile(path.join(projectRoot, 'README.md'), '# M02 fixture\n', 'utf8'),
    writeFile(
...
try {
  ...
  console.log('M02 controlled discovery scenario: PASS');
